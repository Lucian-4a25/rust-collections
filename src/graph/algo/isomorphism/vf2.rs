use super::semantic::{EdgeMatcher, NoSemanticMatch, NodeMatcher};
use crate::graph::{
    visit::{
        GetAdjacencyMatrix, GraphDataAccess, GraphProp, IntoEdgeDirected, IntoNeighborsDirected,
...
where
    G0: NodeIndexable + NodeCount + GetAdjacencyMatrix + IntoNeighborsDirected + GraphProp,
    G1: NodeIndexable + NodeCount + GetAdjacencyMatrix + IntoNeighborsDirected + GraphProp,
{
    ...
    G1: NodeIndexable + NodeCount + GetAdjacencyMatrix + IntoNeighborsDirected + GraphProp,
{
    ...
    where
        G: NodeIndexable + NodeCount + GetAdjacencyMatrix + IntoNeighborsDirected + GraphProp,
    {
        ...
        // for undirected graph, neighbors_directed(_, Outcoming) will output all of neighbors, so
        // it's unneccsarry to call neighbors_directed(_, Incoming)
        if s.graph.is_directed() {
...
where
    G0: NodeIndexable + NodeCount + GraphProp + GetAdjacencyMatrix + IntoNeighborsDirected,
    G1: NodeIndexable + NodeCount + GraphProp + GetAdjacencyMatrix + IntoNeighborsDirected,
    NM: NodeMatcher<G0, G1>,
    EM: EdgeMatcher<G0, G1>,
{
