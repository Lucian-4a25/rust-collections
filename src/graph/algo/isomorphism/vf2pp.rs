...
use crate::graph::visit::{
    GraphBase, GraphDataAccess, GraphProp, IntoEdgeDirected, IntoNeighborsDirected,
...
impl<'a, G0, G1, NM, EM> IsomorphismMatcher<'a, G0, G1, NM, EM>
where
    ...
    EM: EdgeMatcher<G0, G1>,
{
    fn new(vf2pp: VF2PP<G0, G1>, node_matcher: NM, edge_matcher: EM, subgraph: bool) -> Self {
        let mapping = vec![usize::MAX; vf2pp.g0.node_count()];
        let depth = 0usize;
        // g1_cons[i] mean the number of neighbors of `i`, i is node_idx value of g1
        // usize::MAX indicate it already exist in the mapping
        let g1_node_cons = vec![0usize; vf2pp.g1.node_count()];
        let g1_candidate_nodes_iter = vec![];
    ...
    // pub fn match_iter(self) -> impl Iterator<Item = Vec<usize>> + 'a {

    // }
}

impl<'a, G0, G1, NM, EM> Iterator for IsomorphismMatcher<'a, G0, G1, NM, EM>
where
    G0: IntoNeighborsDirected
        + IntoNeighborsUnirected
        + IntoNodeIdentifiers
    ...
    G1: IntoNeighborsDirected + IntoNeighborsUnirected + NodeIndexable + NodeCount + GraphProp,
{
    ...
    for d in [Outcoming, Incoming] {
        ...
        for neighbor in g1.neighbors_directed(m, d) {
            let neighbor_id = g1.to_index(neighbor);
            if neighbor_id != NOT_IN_MAPPING && g1_mapping[neighbor_id] != 0 {
                return false;
            }
        }
    }

    check_r_in_out_new_labels(
        n,
        m,
        g0,
        g1,
...
fn check_r_in_out_new_labels<G0, G1>(
    n: G0::NodeId,
    m: G1::NodeId,
    g0: G0,
    g1: G1,
    ...
    G1: IntoNeighborsUnirected + NodeIndexable + NodeCount,
{
    ...
    let mut label_in_out_tmp = vec![0; max_label + 1];
    let mut label_new_tmp = vec![0; max_label + 1];
    for neighbor in g1.neighbors_undirected(m) {
        let neighbor_id = g1.to_index(neighbor);
        let neighbor_con = g1_node_cons[neighbor_id];
